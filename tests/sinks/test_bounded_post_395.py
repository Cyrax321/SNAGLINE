...
def test_an_overrun_is_reported_not_silently_succeeded(caplog) -> None:
    ...
    with caplog.at_level("ERROR", logger="snagline"), _trickling_urlopen(_STALL, 4):
        sink.emit(_risk())
    # ``logger.exception`` puts the deadline message in the traceback rather
    # than the format string, so search the whole captured output.
...
def test_an_endpoint_failure_is_still_relayed(caplog) -> None:
    ...
    # preserved -- the error is logged fail-open rather than raised.
    with (
        caplog.at_level("ERROR", logger="snagline"),
        mock.patch.object(
            sinks_base._opener,
