...
def test_emit_posts_failure_risk_fields_only() -> None:
    ...
    with mock.patch.object(sinks_base._opener, "open", side_effect=fake_urlopen):
...
def test_emit_never_raises_on_network_failure() -> None:
    with mock.patch.object(
        sinks_base._opener, "open", side_effect=OSError("connection refused")
    ):
...
def test_min_severity_info_passes_everything() -> None:
    posted: list = []
    with mock.patch.object(
        sinks_base._opener, "open", side_effect=_posting_urlopen(posted)
    ):
